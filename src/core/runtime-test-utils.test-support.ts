...
export function makeDriver(
  ...
  startResult: DriverAppStartResult = { status: "ok", activity: "com.example/.MainActivity", exitCode: 0, durationMs: 1 },
  ...
  getDeviceImeState: ReturnType<typeof vi.fn>;enableInputMethod: ReturnType<typeof vi.fn>;setInputMethod: ReturnType<typeof vi.fn>;resetInputMethod: ReturnType<typeof vi.fn>;
  ...
  currentApp: ReturnType<typeof vi.fn>;
  ...
  const observe = vi.fn(async (_options: ObserveOptions) => {
    const next = snapshots.shift();
    if (next === undefined) {
      throw new AutophoneError({
        code: "INTERNAL",
        message: "test driver has no more snapshots",
        retriable: false
      });
    }
    return next;
  ...
  return {
    ...
    getDeviceImeState: vi.fn(async () => imeDriverResult()),enableInputMethod: vi.fn(async () => ({ serial: "emulator-5554", exitCode: 0, durationMs: 1 })),setInputMethod: vi.fn(async () => ({ serial: "emulator-5554", exitCode: 0, durationMs: 1 })),resetInputMethod: vi.fn(async () => ({ serial: "emulator-5554", exitCode: 0, durationMs: 1 })),
    ...
    currentApp,
    startActivity: vi.fn(async () => startResult),
...
export function permissionInspectDriverResult() {
  return {
    serial: "emulator-5554",
    packageFound: true,
    targetSdk: 35,
